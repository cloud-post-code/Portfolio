...
st.caption("Edit brand voice and identity values used in every prompt.")
...
brand_overview = st.text_area("Overview", value=t["brand_overview"], height=120)
...
st.markdown("### Voice & visual profile")
tones = st.text_area("Tones (one per line)", value="\n".join(t["tones"]), height=100)
...
typography_style = st.text_input("Typography style", value=t["typography_style"])
...
st.session_state.theme = {
    ...
    "tones": [line.strip() for line in tones.splitlines() if line.strip()],
...
theme_json = st.text_area(
    "Paste JSON to merge into current theme",
    placeholder='{"brand_name":"...", "tones":["Warm","Grounded"]}',
    height=140,
)
