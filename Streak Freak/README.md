# Streak Freak

Local Flask + SQLite app to **track daily habits**, sleep times, and streaks — with a simple web UI and confetti when you complete every habit for the day.

## What it does

Check off habits by day, log wake/bed hours, view streaks, and celebrate when today is fully complete. All data stays in a local `habits.db` file (gitignored).

## Setup

```bash
cd "Streak Freak"
python3 -m venv .venv
source .venv/bin/activate   # Windows: .venv\Scripts\activate
python3 -m pip install -r requirements.txt
python3 app.py
```

Or one step from this folder: `./run.sh` (after `chmod +x run.sh` once).

**You must leave this terminal running.** If you close it or press Ctrl+C, the site stops and the browser shows `ERR_CONNECTION_REFUSED`.

When the server starts you should see:

`Streak Freak: http://127.0.0.1:5001/`

Open that URL (default port is **5001** — macOS often uses **5000** for AirPlay Receiver).

### `ERR_CONNECTION_REFUSED`

1. Start the app from the `Streak Freak` folder: `python3 app.py` (see lines above).
2. Use **port 5001**: `http://127.0.0.1:5001/` — not `5000` unless you changed it.
3. If you see import errors, run `python3 -m pip install -r requirements.txt` again inside the venv.

Override the port: `STREAK_FREAK_PORT=8080 python3 app.py` (or legacy `HABITTRACKER_PORT`) then open `http://127.0.0.1:8080/`.

Data is stored in `habits.db` in this folder (created on first run).
